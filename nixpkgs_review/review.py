...
class Review:
    ...
    def git_merge(self, commit: str) -> None:
        sh(["git", "merge", "--no-commit", "--no-ff", commit], cwd=self.worktree_dir(), stderr=sys.stdout)
    ...
    def apply_unstaged(self, staged: bool = False) -> None:
        ...
        result = subprocess.run(["git", "apply"], cwd=self.worktree_dir(), input=diff, stderr=sys.stdout)
